...
def get_packer_action(machine, job_slot, num_nw):
        align_score = 0
        act = len(job_slot.slot[:num_nw])  # if no action available, hold

        for i in range(len(job_slot.slot[:num_nw])):
            new_job = job_slot.slot[i]
            if new_job is not None:  # there is a pending job

                avbl_res = machine.avbl_slot[:new_job.len_hat, :]
...
def get_sjf_action(machine, job_slot, num_nw):
        sjf_score = 0
        act = len(job_slot.slot[:num_nw])  # if no action available, hold
        ...
        for i in range(len(job_slot.slot[:num_nw])):
            ...
            if new_job is not None:  # there is a pending job
                ...
                if np.all(res_left[:] >= 0):  # enough resource to allocate
                    ...
                    tmp_sjf_score = 1 / float(new_job.len_hat)
...
def get_packer_sjf_action(machine, job_slot, knob, num_nw):  # knob controls which to favor, 1 to packer, 0 to sjf
...
def get_random_action(job_slot, num_nw):
    num_act = len(job_slot.slot[:num_nw]) + 1  # if no action available,
