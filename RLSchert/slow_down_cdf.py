...
def get_traj(test_type, pa, env, episode_max_length, pg_resume=None, render=False):
    ...
    for _ in range(episode_max_length):
        ...
        if test_type == 'PG':
            a, _, _ = pg_learner.choose_action(ob)

        elif test_type == 'Tetris':
            a = other_agents.get_packer_action(env.machine, env.job_slot, pa.num_nw)

        elif test_type == 'SJF':
            a = other_agents.get_sjf_action(env.machine, env.job_slot, pa.num_nw)

        elif test_type == 'Random':
            a = other_agents.get_random_action(env.job_slot, pa.num_nw)
...
def launch(pa, pg_resume=None, render=False, plot=False, repre='image', end='no_new_job'):
    ...
    if plot:
        ...
        for test_type in test_types:
            ...
            print(test_type)
            print('mean:'+str(np.mean(slow_down_cdf)))
            print('std:'+str(np.std(slow_down_cdf)))
