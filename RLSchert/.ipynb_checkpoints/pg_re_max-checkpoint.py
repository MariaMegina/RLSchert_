...
def launch(pa, pg_resume=None, render=False, repre='image', end='no_new_job'):
    ...
    if pa.pred_err > 0:
        randnum = np.random.randint(-round(pa.max_job_len*pa.pred_err),round(pa.max_job_len*pa.pred_err)+1,nw_len_hat_seqs.shape)
        nw_len_hat_seqs = nw_len_hat_seqs + randnum
        nw_len_hat_seqs = np.clip(nw_len_hat_seqs, 1, pa.max_job_len)
    ...
    for iteration in range(1, pa.num_epochs):
        ...
        for ex in range(pa.num_ex):
            ...
            if ex_counter >= pa.batch_size or ex == pa.num_ex - 1:
                ...
                pg_learners[pa.batch_size].train(all_ob, all_action.reshape((-1,1)), all_adv.reshape((-1,1)))
        ...
        params = pg_learners[pa.batch_size].l_out.state_dict()
        ...
        if iteration % pa.output_freq == 0:
            ...
            torch.save(pg_learners[pa.batch_size].l_out.state_dict(), param_file)
